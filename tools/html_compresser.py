...
if found_src_folder_in_cur_dir:
    html_uncompressed_folder_full_path = os.path.join(
        cur_dir, html_uncompressed_folder_local_path)
    html_compressed_folder_full_path = os.path.join(
        cur_dir, html_compressed_folder_local_path)
else:
    html_uncompressed_folder_full_path = os.path.join(
        os.path.dirname(cur_dir), html_uncompressed_folder_local_path)
    html_compressed_folder_full_path = os.path.join(
        os.path.dirname(cur_dir), html_compressed_folder_local_path)

print("Uncompressed Folder path '{}'".format(
    html_uncompressed_folder_full_path))
...
if not os.path.isdir(html_uncompressed_folder_full_path):
    ...
    print("Created missing dir '{}'".format(
        os.path.basename(html_uncompressed_folder_full_path)))
    print("")
...
for html_file in html_files:
    ...
    html_uncompressed_folder_full_path_with_file = os.path.join(
        html_uncompressed_folder_full_path, html_file)
    html_file_content = ""
    ...
    if html_file_content != "":
        # minify html page
        html_file_content_minified = minify_html.minify(
            html_file_content, minify_js=True, minify_css=True, remove_processing_instructions=True)
        if enable_hml_print:
        ...
        html_compressed_folder_full_path_with_file = os.path.join(
            html_compressed_folder_full_path, html_file.replace(".html", "_compressed.html"))
        ...
        uncompressed_file_size = os.stat(
            html_uncompressed_folder_full_path_with_file).st_size
        compressed_file_size = os.stat(
            html_compressed_folder_full_path_with_file).st_size

        print("Compressed HTML file '{}' reduced size by {:.2f}%".format(
            html_file, abs((compressed_file_size / uncompressed_file_size) * 100 - 100)))

    else:
        print("Error reading file '{}'".format(
            html_uncompressed_folder_full_path_with_file))
